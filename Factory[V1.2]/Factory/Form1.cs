...
namespace Factory
{
    public partial class Form1 : Form
    {
                ...
                case NotifyCollectionChangedAction.Remove:
                    foreach(Product b in e.OldItems)
                    {
                        Program.MainForm.Invoke(
                            new Action(() =>
                            {
                                //looks in the controls of the queuepanel for the first item that has the same name as the removed product. then removes that item from the panel
                                QueuePanel.Controls.Remove( QueuePanel.Controls.OfType<ProductCarrier>().Where(x => x.ProductCarrierName == b.orderName).First());
                            }));
                    }
                    break;
            }
        ...
        private void TabProduction_Click(object sender, EventArgs e)
        {
